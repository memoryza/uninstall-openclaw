#!/usr/bin/env bash
# =============================================================================
# OpenClaw / Clawdbot Uninstaller — Linux
# Usage: bash uninstall-linux.sh [--keep-config]
...
# ── Step 1: Stop & remove systemd user units ─────────────────────────────────
info "Step 1/5  Stopping systemd user units..."
UNITS=(openclaw-gateway clawdbot-gateway)
found_any=false
for unit in "${UNITS[@]}"; do
  if systemctl --user list-unit-files 2>/dev/null | grep -q "$unit"; then
    ...
    [ -f "$SERVICE_FILE" ] && rm -f "$SERVICE_FILE" && success "Removed:  $SERVICE_FILE"
  fi
done
if $found_any; then
  systemctl --user daemon-reload 2>/dev/null
else
  info "No systemd units found — skipping."
fi
...
for unit in openclaw-gateway clawdbot-gateway; do
